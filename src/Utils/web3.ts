...
class Web3Class {
  ...
  static getWeb3Instance = () => {
    if ((Misc.isMobile && !(window as any).ethereum) || !(window as any).ethereum) {
      if (!web3Instance) {
        try {
          const listLink = !(window as any).ethereum ? [] : ['metamask', 'trust']
          const walletConnectProvider: any = new WalletConnectProvider({
            ...
            qrcodeModal: {
              open: (uri: any) => {
                ...
                // clear web3 session when walleconnect modal click close button, if have other case, please use removeWeb3Instance()
              ...
              // clear web3 session when walleconnect auto close, if have other case, please use removeWeb3Instance()
              close: () => {
                setTimeout(() => {
                  if (
    ...
    if (web3Instance) {
      ...
      busdContract = new web3Instance.eth.Contract(busdAbi, busdContractAddress)
  ...
  static changeNetwork = async (chainId: any) => {
    ...
    if (!isNetwork) {
      ...
      try {
        ...
        } else if (error.code === 4902 || Misc.isMobile || !(window as any).ethereum) {
  ...
  static getBalance = async (publicAddress: string) => {
    ...
    try {
      ...
      return {
        eth: roundDown(+(+web3?.utils.fromWei(ethBalance, 'ether')), 3) || 0,
        usdt: roundDown(+(+web3?.utils.fromWei(usdtBalance, type)), 3) || 0,
      }
    } catch (e) {}
    return {}
  }
  ...
  static getBalanceBUSD = async (publicAddress: any) => {
    ...
    try {
      ...
      return {
        busd: roundDown(+(+web3?.utils.fromWei(busdBalance, 'ether')), 3) || 0,
      }
    } catch (e) {}
    return {}
  }
  ...
  static getSPCReturnsByToken = async (publicAddress: any, amount: any) => {
    ...
    return +(+web3?.utils.fromWei(spc, 'ether'))
  }
  ...
  static getTokenReturnsBySPC = async (publicAddress: any, amount: any) => {
    ...
    return +(+web3?.utils.fromWei(token, 'ether'))
  }
  ...
  static getTokenPrice = async (tokenAddress: any) => {
    const web3: any = new Web3(web3Info.SPC_RPC)
    const tokenizeContract = new web3.eth.Contract(
      tokenizeAbi,
      tokenizeContractAddress
    )
  ...
  static swapSPCtoHTC = async (publicAddress: any, amount: any, amountOutMin = 0) => {
    const instance = await Web3Class.getWeb3Instance()
  ...
  static sendTransaction = async (
    ...
    const transactionHash = await new Promise((resolve, reject) => {
      if (type === COIN_SYMBOL.USDT) {
        const type =
          web3Info.USDT_DECIMALS === 18 ? 'ether' : 'mwei'
        instance.usdtContract.methods
          .transfer(
  ...
  static getBalanceOfToken = async (publicAddress: string) => {
    const web3: any = new Web3(web3Info.SPC_RPC)
    const htcContract = new web3.eth.Contract(htcAbi, htcContractAddress)
    const przContract = new web3.eth.Contract(przAbi, przContractAddress)
    const cctContract = new web3.eth.Contract(cctAbi, cctContractAddress)
    const spcBalance = await web3?.eth.getBalance(publicAddress)
    const htcBalance = await htcContract.methods.balanceOf(publicAddress).call()
    const przBalance = await przContract.methods.balanceOf(publicAddress).call()
    const cctBalance = await cctContract.methods.balanceOf(publicAddress).call()
    return {
      spc: roundDown(+(+web3.utils.fromWei(spcBalance, 'ether')), 3) || 0,
  ...
  static sendStaking = async (publicAddress: any, amount: any, nonce: any, signature: any) => {
    const instance = await Web3Class.getWeb3Instance()
    const transactionHash = await new Promise((resolve, reject) => {
  ...
  // usdt -> htc
  static transferTransaction = async (publicAddress: any, adminAddress: any, amount: any) => {
    const instance = await Web3Class.getWeb3Instance()
    const contractExchangeAddress = adminAddress
    const transactionHash = await new Promise((resolve, reject) => {
      const type = web3Info.USDT_DECIMALS === 18 ? 'ether' : 'mwei'
