...
const scheme = (t: any) => yup.object().shape({
  ...
  overview: yup.string().max(4000, t('error_message:validation.max_length', { key: t('registration_course.create.overview'), max: 4000 })),
  price: yup.number().nullable().required(t('error_message:validation.required', { key: t('registration_course.create.price') }))
